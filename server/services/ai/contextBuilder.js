...
const buildCourseContext = ({ course, progress, sectionId, subSectionId }) => {
  ...
  if (sectionId) {
    const selected = sections.find((section) => objectIdEquals(section._id, sectionId))
    if (!selected) throw new ApiError(404, "Section not found in this course", "SECTION_NOT_FOUND")
    sections = [selected]
  }
  if (subSectionId && !sectionId) {
    throw new ApiError(400, "sectionId is required with subSectionId", "VALIDATION_ERROR")
  }

  const completed = new Set((progress?.completedVideos || []).map(String))
  const references = new Map()
  const contextSections = sections.map((section) => {
    references.set(String(section._id), { id: String(section._id), title: text(section.sectionName, 300), type: "section" })
    let lessons = section.subSection.slice(0, LIMITS.lessonsPerSection)
    if (subSectionId) {
      const selected = lessons.find((lesson) => objectIdEquals(lesson._id, subSectionId))
      if (!selected) throw new ApiError(404, "Lesson not found in this section", "LESSON_NOT_FOUND")
      lessons = [selected]
    }
    return {
      ...
      lessons: lessons.map((lesson) => {
        ...
        references.set(item.id, { id: item.id, title: item.title, type: "lesson" })
        return item
      }),
    }
  })
  ...
  const context = {
    ...
    instructions: (course.instructions || []).slice(0, 30).map((item) => text(item, 500)),
    sections: contextSections,
    sourceNotice: "Based on course titles and descriptions; video transcripts are not available.",
  }
  context.sections = context.sections.map((section) => ({
    ...section,
    lessons: section.lessons.map((lesson) => ({ ...lesson, description: lesson.description.slice(0, 1200) })),
  }))
  ...
  if (JSON.stringify(context).length > LIMITS.total) {
    context.sections = context.sections.map((section) => ({
      ...section,
      lessons: section.lessons.map((lesson) => ({ ...lesson, description: lesson.description.slice(0, 400) })),
    }))
  ...
  const contentForHash = {
    ...
    sections: context.sections.map((section) => ({
      ...section,
      lessons: section.lessons.map(({ completed: ignored, ...lesson }) => lesson),
    })),
  }
  const finalSerialized = JSON.stringify(contentForHash).slice(0, LIMITS.total)
  const sourceContentHash = crypto.createHash("sha256").update(finalSerialized).digest("hex")
  const progressHash = crypto
    .createHash("sha256")
    .update([...completed].sort().join(","))
    .digest("hex")
  const totalLessons = [...references.values()].filter((item) => item.type === "lesson").length
  ...
  return {
    ...
    progress: {
      ...
      percentage: totalLessons ? Math.round((completedInScope / totalLessons) * 10000) / 100 : 0,
    },
  }
}
...
const validateReferences = (items, references) =>
  items.map((item) => {
    ...
    if (!trusted || trusted.type !== item.type) {
